import type { WorkoutTemplate } from "../lib/db";

// ═══════════════════════════════════════════════════════════════════════════
// CICLO 1 — ADAPTAÇÃO — glúteo é PRIORIDADE Nº 1.
// 4 estímulos de glúteo/semana (força, médio, unilateral, volume) + 1 dia de
// superior leve/cintura. Foco: aprender padrão, ativar glúteo, secar barriga.
// Cintura fina = só core transverso (sem oblíquo com carga, que engrossa).
// ═══════════════════════════════════════════════════════════════════════════
export const WORKOUT_PLAN: WorkoutTemplate[] = [
  {
    id: "seg-gluteo-mobilidade",
    name: "🍑 Glúteo A · Força",
    dayOfWeek: 1,
    durationMin: 62,
    cycle: "adaptacao",
    exercises: [
      { exerciseId: "cardio-leve-esteira", sets: 1, repsTarget: "5-7min", restSec: 0 },
      { exerciseId: "aquecimento-articular", sets: 1, repsTarget: "5min", restSec: 0 },
      { exerciseId: "ativacao-gluteo-band-walks", sets: 2, repsTarget: "12 cada", restSec: 30, notes: "Ativação — sente o glúteo ligar antes de carregar" },
      { exerciseId: "hip-thrust-barra", sets: 4, repsTarget: "10-12", restSec: 90, notes: "O MAIOR construtor de glúteo. Carga progressiva: some peso quando fizer todas as reps fácil" },
      { exerciseId: "smith-squat", sets: 3, repsTarget: "12", restSec: 90, notes: "Leg press 45° com pés ALTOS = foco glúteo" },
      { exerciseId: "stiff", sets: 3, repsTarget: "10-12", restSec: 75 },
      { exerciseId: "abdutor-maquina", sets: 3, repsTarget: "15", restSec: 60, notes: "Glúteo médio — arredonda a lateral" },
    ],
  },
  {
    id: "ter-cintura-costas",
    name: "Superior leve + Cintura fina",
    dayOfWeek: 2,
    durationMin: 50,
    cycle: "adaptacao",
    exercises: [
      { exerciseId: "cardio-leve-esteira", sets: 1, repsTarget: "5-7min", restSec: 0 },
      { exerciseId: "aquecimento-articular", sets: 1, repsTarget: "5min", restSec: 0 },
      { exerciseId: "supino-inclinado-halteres", sets: 3, repsTarget: "12 (LEVE)", restSec: 60, notes: "Base de busto — carga leve" },
      { exerciseId: "remada-baixa-maquina", sets: 3, repsTarget: "12", restSec: 60, notes: "Postura — multiestação" },
      { exerciseId: "face-pull", sets: 3, repsTarget: "15", restSec: 45, notes: "Crucifixo invertido c/ halteres — abre os ombros" },
      { exerciseId: "dead-bug", sets: 3, repsTarget: "10 cada", restSec: 30 },
      { exerciseId: "prancha", sets: 3, repsTarget: "30-45s", restSec: 45, notes: "Core sem engrossar a cintura" },
    ],
  },
  {
    id: "qua-mobilidade-danca",
    name: "Mobilidade + Dança + 🍑 Glúteo médio",
    dayOfWeek: 3,
    durationMin: 55,
    cycle: "adaptacao",
    exercises: [
      { exerciseId: "aquecimento-articular", sets: 1, repsTarget: "5min", restSec: 0 },
      { exerciseId: "cat-cow", sets: 2, repsTarget: "10", restSec: 0 },
      { exerciseId: "alongamento-flexor-quadril", sets: 1, repsTarget: "1min cada", restSec: 0 },
      { exerciseId: "abdutor-cabo-em-pe", sets: 3, repsTarget: "15 cada", restSec: 30, notes: "Fire hydrant — glúteo médio, arredonda" },
      { exerciseId: "abdutor-band-em-pe", sets: 3, repsTarget: "15 cada", restSec: 30 },
      { exerciseId: "clamshell", sets: 3, repsTarget: "20 cada", restSec: 30 },
      { exerciseId: "ponte-gluteo-band", sets: 3, repsTarget: "20", restSec: 30 },
      { exerciseId: "abdutor-deitada", sets: 3, repsTarget: "20 cada", restSec: 30 },
      { exerciseId: "rebolado-basico", sets: 3, repsTarget: "1min", restSec: 30 },
      { exerciseId: "isolamento-quadril-lateral", sets: 3, repsTarget: "30s cada", restSec: 30 },
    ],
  },
  {
    id: "qui-gluteo-coxa",
    name: "🍑 Glúteo B · Unilateral + Coxa",
    dayOfWeek: 4,
    durationMin: 60,
    cycle: "adaptacao",
    exercises: [
      { exerciseId: "cardio-leve-esteira", sets: 1, repsTarget: "5-7min", restSec: 0 },
      { exerciseId: "aquecimento-articular", sets: 1, repsTarget: "5min", restSec: 0 },
      { exerciseId: "ativacao-gluteo-band-walks", sets: 2, repsTarget: "12 cada", restSec: 30 },
      { exerciseId: "agachamento-bulgaro", sets: 3, repsTarget: "10 cada", restSec: 75, notes: "Glúteo + coxa cheia, corrige assimetria" },
      { exerciseId: "hip-thrust-unilateral", sets: 3, repsTarget: "10 cada", restSec: 60 },
      { exerciseId: "stiff-unilateral", sets: 3, repsTarget: "8 cada", restSec: 60 },
      { exerciseId: "adutora-maquina", sets: 3, repsTarget: "15", restSec: 45, notes: "Coxa interna cheia — silhueta curvilínea" },
    ],
  },
  {
    id: "sex-peitoral-postura",
    name: "🍑 Glúteo C · Volume + Core",
    dayOfWeek: 5,
    durationMin: 52,
    cycle: "adaptacao",
    exercises: [
      { exerciseId: "cardio-leve-esteira", sets: 1, repsTarget: "5-7min", restSec: 0 },
      { exerciseId: "aquecimento-articular", sets: 1, repsTarget: "5min", restSec: 0 },
      { exerciseId: "hip-thrust-barra", sets: 4, repsTarget: "15-20 (bombeamento)", restSec: 45, notes: "Reps altas, carga média — bomba de sangue no glúteo" },
      { exerciseId: "kickback-cabo", sets: 3, repsTarget: "15 cada", restSec: 30, notes: "Coice com caneleira" },
      { exerciseId: "abdutor-band-em-pe", sets: 3, repsTarget: "15 cada", restSec: 30 },
      { exerciseId: "elevacao-pelvica-banco", sets: 3, repsTarget: "12 cada", restSec: 45 },
      { exerciseId: "prancha-lateral", sets: 3, repsTarget: "30s cada", restSec: 30 },
    ],
  },
];
